          ...
          >
            {images.map((image, index) => (
              <SwiperSlide key={image.id}>
                <div className="relative w-full h-44 xs:h-56 sm:h-64 md:h-72 lg:h-80 xl:h-96 2xl:h-[22rem] max-w-full mx-auto overflow-hidden rounded-2xl shadow-2xl transform transition-all duration-500 hover:scale-105 hover:shadow-brand-primary/30 group/card border-4 border-brand-border hover:border-brand-primary">
                  {/* Image */}
                  <img
                    src={image.url}
                    alt={image.alt}
